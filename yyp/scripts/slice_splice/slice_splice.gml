/// @function slice_splice(this:slice, pos:int, len:int)->array<T>
/// @param this:slice
/// @param pos:int
/// @param len:int
var this = argument[0], pos = argument[1], len = argument[2];
var arrayLength = array_length_1d(this[0/* array */]);
if (len < 0 || pos >= arrayLength) return array_create(0);
if (pos < 0) {
	pos += len;
	if (pos < 0) pos = 0;
}
...
for (var _g1 = arrayLength; i < _g1; i++) {
	if (i >= pos && i < var_end) array_hx_push(out, old[i]); else array_hx_push(this[0/* array */], old[i]);
}
return out;
